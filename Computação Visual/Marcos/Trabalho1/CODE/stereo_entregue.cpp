    ...
    if(i == 0 && j == 0){
        ...
        if(flag){
        	if(j > 0){
        	    ...
        	    CV_IMAGE_ELEM(esq, uchar, pass, i*3  ) = CV_IMAGE_ELEM(dir, uchar, pass, j*3  );
		        CV_IMAGE_ELEM(esq, uchar, pass, i*3+1) = CV_IMAGE_ELEM(dir, uchar, pass, j*3+1);
		        CV_IMAGE_ELEM(esq, uchar, pass, i*3+2) = CV_IMAGE_ELEM(dir, uchar, pass, j*3+2);    
	        }
    	}
    }else if(j == 0){
        ...
        if(flag){
        	if(j > 0){
		        ...
		        CV_IMAGE_ELEM(esq, uchar, pass, (i-1)*3  ) = CV_IMAGE_ELEM(dir, uchar, pass, j*3  );
		        CV_IMAGE_ELEM(esq, uchar, pass, (i-1)*3+1) = CV_IMAGE_ELEM(dir, uchar, pass, j*3+1);
		        CV_IMAGE_ELEM(esq, uchar, pass, (i-1)*3+2) = CV_IMAGE_ELEM(dir, uchar, pass, j*3+2);    
	        }
	    }
